...
import { runFileAgent } from './file-agent.js';
...
function Header() {
  return React.createElement(
    ...
    React.createElement(Text, { color: 'cyan', bold: true }, 'File Agent')
  );
}
  ...
  if (items.length === 0) {
    return React.createElement(
      ...
      React.createElement(
        Text,
        { color: 'gray' },
        'Type a file request and press Enter. Press Ctrl+C to exit.'
      )
    );
  }
...
function Prompt({ value, isRunning }) {
  const inputText = value.length > 0 ? value : 'Ask about sample files...';
...
function StatusBar({ isRunning }) {
  const status = isRunning ? `status: running file agent with ${getLlmMode()}` : `status: idle (${getLlmMode()})`;
...
function App() {
  ...
  async function submitRequest(request) {
    ...
    // 파일 tool 선택, 파일 읽기/검색, 종료 조건은 file-agent.js가 담당합니다.
    setInput('');
    setIsRunning(true);
    setHistory((current) => [
      ...current,
    ...
    try {
      const result = await runFileAgent(trimmed, (item) => {
  ...
  useInput((character, key) => {
    ...
    if (key.return) {
      // Enter를 누르면 하나의 file agent loop가 시작됩니다.
      submitRequest(input);
      return;
    }
