export async function mockLlm(prompt) {
  ...
  if (prompt.includes('Step: answer_with_tool_result')) {
    const toolName = extractSection(prompt, 'Tool name:');
    const toolResult = extractSection(prompt, 'Tool result:');

    return [
      'Final answer from mock LLM:',
      `I used ${toolName} to answer your request.`,
      `Tool result: ${toolResult}`,
      'This answer was generated after putting the tool result back into the LLM context.'
    ].join('\n');
  }
