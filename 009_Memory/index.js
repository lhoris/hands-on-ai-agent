...
import { runMemoryWorkflow } from './memory-workflow.js';
...
function Header() {
  return React.createElement(
    ...
    React.createElement(Text, { color: 'cyan', bold: true }, 'Memory Workflow')
  );
}
...
function Prompt({ value, isRunning }) {
  const inputText = value.length > 0 ? value : 'Ask with memory...';
...
function StatusBar({ isRunning }) {
  const status = isRunning ? `status: running memory workflow with ${getLlmMode()}` : `status: idle (${getLlmMode()})`;
...
function App() {
  ...
  async function submitRequest(request) {
    ...
    // memory 조회, LLM 호출, memory 저장은 memory-workflow.js가 담당합니다.
    setInput('');
    setIsRunning(true);
    setHistory((current) => [
      ...current,
    ...
    try {
      const result = await runMemoryWorkflow(trimmed, (item) => {
  ...
  useInput((character, key) => {
    ...
    if (key.return) {
      // Enter를 누르면 memory workflow가 한 번 실행됩니다.
      submitRequest(input);
      return;
    }
